...
@[target]
theorem lfpApprox_monotone : Monotone (lfpApprox f x) := by sorry

@[target]
theorem le_lfpApprox {a : Ordinal} : x ≤ lfpApprox f x a := by sorry

@[target]
theorem lfpApprox_add_one (h : x ≤ f x) (a : Ordinal) :
    lfpApprox f x (a+1) = f (lfpApprox f x a) := by sorry

@[target]
theorem lfpApprox_mono_left : Monotone (lfpApprox : (α →o α) → _) := by sorry

@[target]
theorem lfpApprox_mono_mid : Monotone (lfpApprox f) := by sorry
...
theorem lfpApprox_eq_of_mem_fixedPoints {a b : Ordinal} (h_init : x ≤ f x) (h_ab : a ≤ b)
    (h : lfpApprox f x a ∈ fixedPoints f) : lfpApprox f x b = lfpApprox f x a := by sorry
...
theorem exists_lfpApprox_eq_lfpApprox : ∃ a < ord <| succ #α, ∃ b < ord <| succ #α,
    a ≠ b ∧ lfpApprox f x a = lfpApprox f x b := by sorry
...
lemma lfpApprox_mem_fixedPoints_of_eq {a b c : Ordinal}
    ...
    lfpApprox f x c ∈ fixedPoints f := by sorry
...
theorem lfpApprox_le_of_mem_fixedPoints {a : α}
    (h_a : a ∈ fixedPoints f) (h_le_init : x ≤ a) (i : Ordinal) : lfpApprox f x i ≤ a := by sorry
...
@[target]
theorem lfp_mem_range_lfpApprox : f.lfp ∈ Set.range (lfpApprox f ⊥) := by sorry
...
@[target]
theorem gfpApprox_antitone : Antitone (gfpApprox f x) := by sorry
...
@[target]
theorem gfpApprox_mono_left : Monotone (gfpApprox : (α →o α) → _) := by sorry

@[target]
theorem gfpApprox_mono_mid : Monotone (gfpApprox f) := by sorry
...
theorem gfpApprox_eq_of_mem_fixedPoints {a b : Ordinal} (h_init : f x ≤ x) (h_ab : a ≤ b)
    (h : gfpApprox f x a ∈ fixedPoints f) : gfpApprox f x b = gfpApprox f x a := by sorry

/-- There are distinct indices smaller than the successor of the domain's cardinality
yielding the same value -/
@[target]
theorem exists_gfpApprox_eq_gfpApprox : ∃ a < ord <| succ #α, ∃ b < ord <| succ #α,
    a ≠ b ∧ gfpApprox f x a = gfpApprox f x b := by sorry
...
@[target]
theorem gfp_mem_range_gfpApprox : f.gfp ∈ Set.range (gfpApprox f ⊤) := by sorry
