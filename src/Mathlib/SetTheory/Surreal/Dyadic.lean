...
@[target, simp]
theorem powHalf_zero : powHalf 0 = 1 := by sorry

@[target]
theorem powHalf_leftMoves (n) : (powHalf n).LeftMoves = PUnit := by sorry

@[target]
theorem powHalf_zero_rightMoves : (powHalf 0).RightMoves = PEmpty := by sorry

@[target]
theorem powHalf_succ_rightMoves (n) : (powHalf (n + 1)).RightMoves = PUnit := by sorry

@[simp]
theorem powHalf_moveLeft (n i) : (powHalf n).moveLeft i = 0 := by cases n <;> cases i <;> rfl

@[target, simp]
theorem powHalf_succ_moveRight (n i) : (powHalf (n + 1)).moveRight i = powHalf n := by sorry
...
@[target]
theorem numeric_powHalf (n) : (powHalf n).Numeric := by sorry

@[target]
theorem powHalf_succ_lt_powHalf (n : ℕ) : powHalf (n + 1) < powHalf n := by sorry

@[target]
theorem powHalf_succ_le_powHalf (n : ℕ) : powHalf (n + 1) ≤ powHalf n := by sorry

@[target]
theorem powHalf_le_one (n : ℕ) : powHalf n ≤ 1 := by sorry

theorem powHalf_succ_lt_one (n : ℕ) : powHalf (n + 1) < 1 :=
  (powHalf_succ_lt_powHalf n).trans_le <| powHalf_le_one n

@[target]
theorem powHalf_pos (n : ℕ) : 0 < powHalf n := by sorry

@[target]
theorem zero_le_powHalf (n : ℕ) : 0 ≤ powHalf n := by sorry

@[target]
theorem add_powHalf_succ_self_eq_powHalf (n) : powHalf (n + 1) + powHalf (n + 1) ≈ powHalf n := by sorry

@[target]
theorem half_add_half_equiv_one : powHalf 1 + powHalf 1 ≈ 1 := by sorry
...
@[target, simp]
theorem double_powHalf_succ_eq_powHalf (n : ℕ) : 2 * powHalf (n + 1) = powHalf n := by sorry

@[target, simp]
theorem nsmul_pow_two_powHalf (n : ℕ) : 2 ^ n * powHalf n = 1 := by sorry

@[target, simp]
theorem nsmul_pow_two_powHalf' (n k : ℕ) : 2 ^ n * powHalf (n + k) = powHalf k := by sorry

@[target]
theorem zsmul_pow_two_powHalf (m : ℤ) (n k : ℕ) :
    (m * 2 ^ n) * powHalf (n + k) = m * powHalf k := by sorry

@[target]
theorem dyadic_aux {m₁ m₂ : ℤ} {y₁ y₂ : ℕ} (h₂ : m₁ * 2 ^ y₁ = m₂ * 2 ^ y₂) :
    m₁ * powHalf y₂ = m₂ * powHalf y₁ := by sorry
