...
theorem impartial_def {G : PGame} :
    G.Impartial ↔ G ≈ -G ∧ (∀ i, Impartial (G.moveLeft i)) ∧ ∀ j, Impartial (G.moveRight j) := by sorry
...
@[target]
theorem neg_equiv_self (G : PGame) [h : G.Impartial] : G ≈ -G := by sorry

@[target, simp]
theorem mk'_neg_equiv_self (G : PGame) [G.Impartial] : -(⟦G⟧ : Game) = ⟦G⟧ := by sorry
...
@[target]
theorem impartial_congr {G H : PGame} (e : G ≡r H) [G.Impartial] : H.Impartial := by sorry
...
@[target]
theorem equiv_or_fuzzy_zero : G ≈ 0 ∨ G ‖ 0 := by sorry

@[target, simp]
theorem not_equiv_zero_iff : ¬ G ≈ 0 ↔ G ‖ 0 := by sorry
...
@[target]
theorem equiv_iff_add_equiv_zero (H : PGame) : H ≈ G ↔ H + G ≈ 0 := by sorry

/-- This lemma doesn't require `H` to be impartial. -/
@[target]
theorem equiv_iff_add_equiv_zero' (H : PGame) : G ≈ H ↔ G + H ≈ 0 := by sorry

@[target]
theorem le_zero_iff {G : PGame} [G.Impartial] : G ≤ 0 ↔ 0 ≤ G := by sorry

theorem lf_zero_iff {G : PGame} [G.Impartial] : G ⧏ 0 ↔ 0 ⧏ G := by
  rw [← zero_lf_neg_iff, lf_congr_right (neg_equiv_self G)]

@[target]
theorem equiv_zero_iff_le : (G ≈ 0) ↔ G ≤ 0 := by sorry

@[target]
theorem fuzzy_zero_iff_lf : G ‖ 0 ↔ G ⧏ 0 := by sorry

@[target]
theorem equiv_zero_iff_ge : (G ≈ 0) ↔ 0 ≤ G := by sorry

@[target]
theorem fuzzy_zero_iff_gf : G ‖ 0 ↔ 0 ⧏ G := by sorry

@[target]
theorem forall_leftMoves_fuzzy_iff_equiv_zero : (∀ i, G.moveLeft i ‖ 0) ↔ G ≈ 0 := by sorry

@[target]
theorem forall_rightMoves_fuzzy_iff_equiv_zero : (∀ j, G.moveRight j ‖ 0) ↔ G ≈ 0 := by sorry
