...
@[target, simp]
theorem mk_type (α A) : «Type» ⟨α, A⟩ = α := by sorry
...
@[target, simp]
theorem eta : ∀ x : PSet, mk x.Type x.Func = x := by sorry
...
theorem equiv_iff :
    ∀ {x y : PSet},
      Equiv x y ↔ (∀ i, ∃ j, Equiv (x.Func i) (y.Func j)) ∧ ∀ j, ∃ i, Equiv (x.Func i) (y.Func j) := by sorry

@[target]
theorem Equiv.exists_left {x y : PSet} (h : Equiv x y) : ∀ i, ∃ j, Equiv (x.Func i) (y.Func j) := by sorry
...
@[target]
theorem Equiv.ext : ∀ x y : PSet, Equiv x y ↔ x ⊆ y ∧ y ⊆ x := by sorry

@[target]
theorem Subset.congr_left : ∀ {x y z : PSet}, Equiv x y → (x ⊆ z ↔ y ⊆ z) := by sorry
...
@[target]
theorem Mem.mk {α : Type u} (A : α → PSet) (a : α) : A a ∈ mk α A := by sorry

@[target]
theorem func_mem (x : PSet) (i : x.Type) : x.Func i ∈ x := by sorry

@[target]
theorem Mem.ext : ∀ {x y : PSet.{u}}, (∀ w : PSet.{u}, w ∈ x ↔ w ∈ y) → Equiv x y := by sorry

@[target]
theorem Mem.congr_right : ∀ {x y : PSet.{u}}, Equiv x y → ∀ {w : PSet.{u}}, w ∈ x ↔ w ∈ y := by sorry
...
@[target]
theorem Mem.congr_left : ∀ {x y : PSet.{u}}, Equiv x y → ∀ {w : PSet.{u}}, x ∈ w ↔ y ∈ w := by sorry

@[target]
theorem mem_of_subset {x y z : PSet} : x ⊆ y → z ∈ x → z ∈ y := by sorry

@[target]
theorem subset_iff {x y : PSet} : x ⊆ y ↔ ∀ ⦃z⦄, z ∈ x → z ∈ y := by sorry
...
@[target]
theorem not_subset_of_mem {x y : PSet} (h : x ∈ y) : ¬ y ⊆ x := by sorry

@[target]
theorem not_mem_of_subset {x y : PSet} (h : x ⊆ y) : y ∉ x := by sorry
...
@[target, simp]
theorem nonempty_toSet_iff {u : PSet} : u.toSet.Nonempty ↔ u.Nonempty := by sorry

@[target]
theorem nonempty_type_iff_nonempty {x : PSet} : Nonempty x.Type ↔ PSet.Nonempty x := by sorry

@[target]
theorem nonempty_of_nonempty_type (x : PSet) [h : Nonempty x.Type] : PSet.Nonempty x := by sorry
...
@[target, simp]
theorem not_mem_empty (x : PSet.{u}) : x ∉ (∅ : PSet.{u}) := by sorry

@[target, simp]
theorem toSet_empty : toSet ∅ = ∅ := by sorry

@[target, simp]
theorem empty_subset (x : PSet.{u}) : (∅ : PSet) ⊆ x := by sorry
...
@[target, simp]
theorem mem_insert_iff : ∀ {x y z : PSet.{u}}, x ∈ insert y z ↔ Equiv x y ∨ x ∈ z := by sorry

@[target]
theorem mem_insert (x y : PSet) : x ∈ insert x y := by sorry

@[target]
theorem mem_insert_of_mem {y z : PSet} (x) (h : z ∈ y) : z ∈ insert x y := by sorry

@[target, simp]
theorem mem_singleton {x y : PSet} : x ∈ ({y} : PSet) ↔ Equiv x y := by sorry
...
@[target, simp]
theorem mem_powerset : ∀ {x y : PSet}, y ∈ powerset x ↔ y ⊆ x := by sorry
...
@[target, simp]
theorem mem_sUnion : ∀ {x y : PSet.{u}}, y ∈ ⋃₀ x ↔ ∃ z ∈ x, y ∈ z := by sorry

@[target, simp]
theorem toSet_sUnion (x : PSet.{u}) : (⋃₀ x).toSet = ⋃₀ (toSet '' x.toSet) := by sorry
...
theorem mem_image {f : PSet.{u} → PSet.{u}} (H : ∀ x y, Equiv x y → Equiv (f x) (f y)) :
    ∀ {x y : PSet.{u}}, y ∈ image f x ↔ ∃ z ∈ x, Equiv y (f z) := by sorry
...
@[target, simp]
theorem mk_out : ∀ x : ZFSet, mk x.out = x := by sorry
...
lemma Definable₂.mk_out {f : ZFSet.{u} → ZFSet.{u} → ZFSet.{u}} [Definable₂ f]
    {x y : PSet} :
    .mk (out f x y) = f (.mk x) (.mk y) := by sorry
...
set_option linter.deprecated false in
@[target, deprecated "No deprecation message was provided." (since := "2024-09-02")]
theorem eval_val {n f x} :
    (@eval (n + 1) f : ZFSet → OfArity ZFSet ZFSet n) ⟦x⟧ = eval n (Resp.f f x) := by sorry
...
theorem Definable.eq {n} :
    ∀ (s : OfArity ZFSet.{u} ZFSet.{u} n) [H : Definable n s], (@Definable.Resp n s H).eval _ = s := by sorry
...
@[target]
theorem sound {x y : PSet} (h : PSet.Equiv x y) : mk x = mk y := by sorry

theorem exact {x y : PSet} : mk x = mk y → PSet.Equiv x y :=
  Quotient.exact

set_option linter.deprecated false in
@[target, deprecated "No deprecation message was provided." (since := "2024-09-02"), simp]
theorem eval_mk {n f x} :
    (@Resp.eval (n + 1) f : ZFSet → OfArity ZFSet ZFSet n) (mk x) = Resp.eval n (Resp.f f x) := by sorry
...
@[target]
theorem nonempty_def (u : ZFSet) : u.Nonempty ↔ ∃ x, x ∈ u := by sorry

@[target]
theorem nonempty_of_mem {x u : ZFSet} (h : x ∈ u) : u.Nonempty := by sorry

@[target, simp]
theorem nonempty_toSet_iff {u : ZFSet} : u.toSet.Nonempty ↔ u.Nonempty := by sorry
...
@[target, simp]
theorem subset_iff : ∀ {x y : PSet}, mk x ⊆ mk y ↔ x ⊆ y := by sorry

@[target, simp]
theorem toSet_subset_iff {x y : ZFSet} : x.toSet ⊆ y.toSet ↔ x ⊆ y := by sorry

@[target, ext]
theorem ext {x y : ZFSet.{u}} : (∀ z : ZFSet.{u}, z ∈ x ↔ z ∈ y) → x = y := by sorry
...
@[target, simp]
theorem not_mem_empty (x) : x ∉ (∅ : ZFSet.{u}) := by sorry

@[target, simp]
theorem toSet_empty : toSet ∅ = ∅ := by sorry

@[target, simp]
theorem empty_subset (x : ZFSet.{u}) : (∅ : ZFSet) ⊆ x := by sorry

@[simp]
theorem not_nonempty_empty : ¬ZFSet.Nonempty ∅ := by simp [ZFSet.Nonempty]

@[target, simp]
theorem nonempty_mk_iff {x : PSet} : (mk x).Nonempty ↔ x.Nonempty := by sorry

@[target]
theorem eq_empty (x : ZFSet.{u}) : x = ∅ ↔ ∀ y : ZFSet.{u}, y ∉ x := by sorry

@[target]
theorem eq_empty_or_nonempty (u : ZFSet) : u = ∅ ∨ u.Nonempty := by sorry
...
@[target, simp]
theorem mem_insert_iff {x y z : ZFSet.{u}} : x ∈ insert y z ↔ x = y ∨ x ∈ z := by sorry

@[target]
theorem mem_insert (x y : ZFSet) : x ∈ insert x y := by sorry

@[target]
theorem mem_insert_of_mem {y z : ZFSet} (x) (h : z ∈ y) : z ∈ insert x y := by sorry

@[target, simp]
theorem toSet_insert (x y : ZFSet) : (insert x y).toSet = insert x y.toSet := by sorry

@[target, simp]
theorem mem_singleton {x y : ZFSet.{u}} : x ∈ @singleton ZFSet.{u} ZFSet.{u} _ y ↔ x = y := by sorry
...
@[target]
theorem singleton_nonempty (u : ZFSet) : ZFSet.Nonempty {u} := by sorry

@[target]
theorem mem_pair {x y z : ZFSet.{u}} : x ∈ ({y, z} : ZFSet) ↔ x = y ∨ x = z := by sorry

@[target, simp]
theorem pair_eq_singleton (x : ZFSet) : {x, x} = ({x} : ZFSet) := by sorry

@[target, simp]
theorem pair_eq_singleton_iff {x y z : ZFSet} : ({x, y} : ZFSet) = {z} ↔ x = z ∧ y = z := by sorry

@[target, simp]
theorem singleton_eq_pair_iff {x y z : ZFSet} : ({x} : ZFSet) = {y, z} ↔ x = y ∧ x = z := by sorry
...
@[target, simp]
theorem omega_succ {n} : n ∈ omega.{u} → insert n n ∈ omega.{u} := by sorry
...
theorem mem_sep {p : ZFSet.{u} → Prop} {x y : ZFSet.{u}} :
    y ∈ ZFSet.sep p x ↔ y ∈ x ∧ p y := by sorry

@[target, simp]
theorem sep_empty (p : ZFSet → Prop) : (∅ : ZFSet).sep p = ∅ := by sorry
...
@[target, simp]
theorem mem_powerset {x y : ZFSet.{u}} : y ∈ powerset x ↔ y ⊆ x := by sorry

@[target]
theorem sUnion_lem {α β : Type u} (A : α → PSet) (B : β → PSet) (αβ : ∀ a, ∃ b, Equiv (A a) (B b)) :
    ∀ a, ∃ b, Equiv ((sUnion ⟨α, A⟩).Func a) ((sUnion ⟨β, B⟩).Func b) := by sorry
...
@[target, simp]
theorem mem_sUnion {x y : ZFSet.{u}} : y ∈ ⋃₀ x ↔ ∃ z ∈ x, y ∈ z := by sorry

@[target]
theorem mem_sInter {x y : ZFSet} (h : x.Nonempty) : y ∈ ⋂₀ x ↔ ∀ z ∈ x, y ∈ z := by sorry

@[target, simp]
theorem sUnion_empty : ⋃₀ (∅ : ZFSet.{u}) = ∅ := by sorry

@[target, simp]
theorem sInter_empty : ⋂₀ (∅ : ZFSet) = ∅ := by sorry
...
@[target]
theorem not_mem_sInter_of_not_mem {x y z : ZFSet} (hy : ¬y ∈ z) (hz : z ∈ x) : ¬y ∈ ⋂₀ x := by sorry
...
@[target, simp]
theorem sInter_singleton {x : ZFSet.{u}} : ⋂₀ ({x} : ZFSet) = x := by sorry

@[target, simp]
theorem toSet_sUnion (x : ZFSet.{u}) : (⋃₀ x).toSet = ⋃₀ (toSet '' x.toSet) := by sorry
...
@[target]
theorem singleton_injective : Function.Injective (@singleton ZFSet ZFSet _) := by sorry

@[target, simp]
theorem singleton_inj {x y : ZFSet} : ({x} : ZFSet) = {y} ↔ x = y := by sorry
...
@[target, simp]
theorem toSet_union (x y : ZFSet.{u}) : (x ∪ y).toSet = x.toSet ∪ y.toSet := by sorry

@[target, simp]
theorem toSet_inter (x y : ZFSet.{u}) : (x ∩ y).toSet = x.toSet ∩ y.toSet := by sorry

@[target, simp]
theorem toSet_sdiff (x y : ZFSet.{u}) : (x \ y).toSet = x.toSet \ y.toSet := by sorry

@[target, simp]
theorem mem_union {x y z : ZFSet.{u}} : z ∈ x ∪ y ↔ z ∈ x ∨ z ∈ y := by sorry

@[target, simp]
theorem mem_inter {x y z : ZFSet.{u}} : z ∈ x ∩ y ↔ z ∈ x ∧ z ∈ y := by sorry
...
@[target, simp]
theorem sUnion_pair {x y : ZFSet.{u}} : ⋃₀ ({x, y} : ZFSet.{u}) = x ∪ y := by sorry

@[target]
theorem mem_wf : @WellFounded ZFSet (· ∈ ·) := by sorry

/-- Induction on the `∈` relation. -/
@[target, elab_as_elim]
theorem inductionOn {p : ZFSet → Prop} (x) (h : ∀ x, (∀ y ∈ x, p y) → p x) : p x := by sorry
...
@[target]
theorem mem_asymm {x y : ZFSet} : x ∈ y → y ∉ x := by sorry

@[target]
theorem mem_irrefl (x : ZFSet) : x ∉ x := by sorry

@[target]
theorem not_subset_of_mem {x y : ZFSet} (h : x ∈ y) : ¬ y ⊆ x := by sorry
...
@[target]
theorem image.mk (f : ZFSet.{u} → ZFSet.{u}) [Definable₁ f] (x) {y} : y ∈ x → f y ∈ image f x := by sorry

@[target, simp]
theorem mem_image {f : ZFSet.{u} → ZFSet.{u}} [Definable₁ f] {x y : ZFSet.{u}} :
    y ∈ image f x ↔ ∃ z ∈ x, f z = y := by sorry
...
theorem mem_range {α} [Small.{u} α] {f : α → ZFSet.{u}} {x : ZFSet.{u}} :
    x ∈ range f ↔ x ∈ Set.range f := by sorry

@[target, simp]
theorem toSet_range {α} [Small.{u} α] (f : α → ZFSet.{u}) :
    (range f).toSet = Set.range f := by sorry
...
@[target, simp]
theorem toSet_pair (x y : ZFSet.{u}) : (pair x y).toSet = {{x}, {x, y}} := by sorry
...
theorem mem_pairSep {p} {x y z : ZFSet.{u}} :
    z ∈ pairSep p x y ↔ ∃ a ∈ x, ∃ b ∈ y, z = pair a b ∧ p a b := by sorry
...
@[target, simp]
theorem mem_funs {x y f : ZFSet.{u}} : f ∈ funs x y ↔ IsFunc x y f := by sorry
...
@[target]
theorem hereditarily_iff : Hereditarily p x ↔ p x ∧ ∀ y ∈ x, Hereditarily p y := by sorry

@[target]
theorem Hereditarily.self (h : x.Hereditarily p) : p x := by sorry

@[target]
theorem Hereditarily.mem (h : x.Hereditarily p) (hy : y ∈ x) : y.Hereditarily p := by sorry

@[target]
theorem Hereditarily.empty : Hereditarily p x → p ∅ := by sorry
...
@[target, ext]
theorem ext {x y : Class.{u}} : (∀ z : ZFSet.{u}, x z ↔ y z) → x = y := by sorry
...
@[target]
theorem mem_def (A B : Class.{u}) : A ∈ B ↔ ∃ x : ZFSet, ↑x = A ∧ B x := by sorry

@[target, simp]
theorem not_mem_empty (x : Class.{u}) : x ∉ (∅ : Class.{u}) := by sorry
...
@[target, simp]
theorem mem_univ {A : Class.{u}} : A ∈ univ.{u} ↔ ∃ x : ZFSet.{u}, ↑x = A := by sorry
...
@[target]
theorem eq_univ_iff_forall {A : Class.{u}} : A = univ ↔ ∀ x : ZFSet, A x := by sorry

theorem eq_univ_of_forall {A : Class.{u}} : (∀ x : ZFSet, A x) → A = univ :=
  Set.eq_univ_of_forall

@[target]
theorem mem_wf : @WellFounded Class.{u} (· ∈ ·) := by sorry
...
@[target]
theorem mem_asymm {x y : Class} : x ∈ y → y ∉ x := by sorry

@[target]
theorem mem_irrefl (x : Class) : x ∉ x := by sorry
...
@[target, simp]
theorem congToClass_empty : congToClass ∅ = ∅ := by sorry
...
@[target, simp]
theorem classToCong_empty : classToCong ∅ = ∅ := by sorry
...
@[target]
theorem ofSet.inj {x y : ZFSet.{u}} (h : (x : Class.{u}) = y) : x = y := by sorry

@[target, simp]
theorem toSet_of_ZFSet (A : Class.{u}) (x : ZFSet.{u}) : ToSet A x ↔ A x := by sorry

@[target, simp, norm_cast]
theorem coe_mem {x : ZFSet.{u}} {A : Class.{u}} : ↑x ∈ A ↔ A x := by sorry

@[target, simp]
theorem coe_apply {x y : ZFSet.{u}} : (y : Class.{u}) x ↔ x ∈ y := by sorry

@[target, simp, norm_cast]
theorem coe_subset (x y : ZFSet.{u}) : (x : Class.{u}) ⊆ y ↔ x ⊆ y := by sorry

@[target, simp, norm_cast]
theorem coe_sep (p : Class.{u}) (x : ZFSet.{u}) :
    (ZFSet.sep p x : Class) = { y ∈ x | p y } := by sorry

@[target, simp, norm_cast]
theorem coe_empty : ↑(∅ : ZFSet.{u}) = (∅ : Class.{u}) := by sorry

@[target, simp, norm_cast]
theorem coe_insert (x y : ZFSet.{u}) : ↑(insert x y) = @insert ZFSet.{u} Class.{u} _ x y := by sorry

@[target, simp, norm_cast]
theorem coe_union (x y : ZFSet.{u}) : ↑(x ∪ y) = (x : Class.{u}) ∪ y := by sorry

@[target, simp, norm_cast]
theorem coe_inter (x y : ZFSet.{u}) : ↑(x ∩ y) = (x : Class.{u}) ∩ y := by sorry

@[target, simp, norm_cast]
theorem coe_diff (x y : ZFSet.{u}) : ↑(x \ y) = (x : Class.{u}) \ y := by sorry

@[target, simp, norm_cast]
theorem coe_powerset (x : ZFSet.{u}) : ↑x.powerset = powerset.{u} x := by sorry
...
@[target, simp]
theorem sUnion_apply {x : Class} {y : ZFSet} : (⋃₀ x) y ↔ ∃ z : ZFSet, x z ∧ y ∈ z := by sorry
...
@[target, simp]
theorem mem_sUnion {x y : Class.{u}} : y ∈ ⋃₀ x ↔ ∃ z, z ∈ x ∧ y ∈ z := by sorry

@[target]
theorem sInter_apply {x : Class.{u}} {y : ZFSet.{u}} : (⋂₀ x) y ↔ ∀ z : ZFSet.{u}, x z → y ∈ z := by sorry
...
@[target]
theorem mem_of_mem_sInter {x y z : Class} (hy : y ∈ ⋂₀ x) (hz : z ∈ x) : y ∈ z := by sorry

@[target]
theorem mem_sInter {x y : Class.{u}} (h : x.Nonempty) : y ∈ ⋂₀ x ↔ ∀ z, z ∈ x → y ∈ z := by sorry

@[target, simp]
theorem sUnion_empty : ⋃₀ (∅ : Class.{u}) = (∅ : Class.{u}) := by sorry

@[target, simp]
theorem sInter_empty : ⋂₀ (∅ : Class.{u}) = univ := by sorry

/-- An induction principle for sets. If every subset of a class is a member, then the class is
  universal. -/
@[target]
theorem eq_univ_of_powerset_subset {A : Class} (hA : powerset A ⊆ A) : A = univ := by sorry
...
@[target]
theorem fval_ex (F A : Class.{u}) : F ′ A ∈ univ.{u} := by sorry
...
theorem map_fval {f : ZFSet.{u} → ZFSet.{u}} [Definable₁ f] {x y : ZFSet.{u}}
    (h : y ∈ x) : (ZFSet.map f x ′ y : Class.{u}) = f y := by sorry
...
@[target]
theorem choice_isFunc (h : ∅ ∉ x) : IsFunc x (⋃₀ x) (choice x) := by sorry
