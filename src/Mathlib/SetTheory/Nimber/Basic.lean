...
@[target, simp]
theorem toOrdinal_symm_eq : Nimber.toOrdinal.symm = Ordinal.toNimber := by sorry

@[target, simp]
theorem toOrdinal_toNimber (a : Nimber) : ∗(Nimber.toOrdinal a) = a := by sorry

@[target]
theorem lt_wf : @WellFounded Nimber (· < ·) := by sorry
...
@[target, simp]
theorem toOrdinal_one : toOrdinal 1 = 1 := by sorry

@[target, simp]
theorem toOrdinal_eq_zero {a} : toOrdinal a = 0 ↔ a = 0 := by sorry

@[target, simp]
theorem toOrdinal_eq_one {a} : toOrdinal a = 1 ↔ a = 1 := by sorry

@[target, simp]
theorem toOrdinal_max (a b : Nimber) : toOrdinal (max a b) = max (toOrdinal a) (toOrdinal b) := by sorry

@[target, simp]
theorem toOrdinal_min (a b : Nimber) : toOrdinal (min a b) = min (toOrdinal a) (toOrdinal b) := by sorry

@[target]
theorem succ_def (a : Nimber) : succ a = ∗(toOrdinal a + 1) := by sorry
...
@[target]
theorem induction {p : Nimber → Prop} : ∀ (i) (_ : ∀ j, (∀ k, k < j → p k) → p j), p i := by sorry
...
@[target]
theorem lt_one_iff_zero {a : Nimber} : a < 1 ↔ a = 0 := by sorry

@[target]
theorem one_le_iff_ne_zero {a : Nimber} : 1 ≤ a ↔ a ≠ 0 := by sorry

@[target]
theorem eq_nat_of_le_nat {a : Nimber} {b : ℕ} (h : a ≤ ∗b) : ∃ c : ℕ, a = ∗c := by sorry
...
@[target]
theorem not_bddAbove_compl_of_small (s : Set Nimber.{u}) [Small.{u} s] : ¬ BddAbove sᶜ := by sorry

end Nimber

@[target]
theorem not_small_nimber : ¬ Small.{u} Nimber.{max u v} := by sorry
...
@[target, simp]
theorem toNimber_symm_eq : toNimber.symm = Nimber.toOrdinal := by sorry

@[target, simp]
theorem toNimber_toOrdinal (a : Ordinal) : Nimber.toOrdinal (∗a) = a := by sorry

@[target, simp]
theorem toNimber_zero : ∗0 = 0 := by sorry
...
@[target, simp]
theorem toNimber_eq_zero {a} : ∗a = 0 ↔ a = 0 := by sorry

@[target, simp]
theorem toNimber_eq_one {a} : ∗a = 1 ↔ a = 1 := by sorry
...
@[target, simp]
theorem toNimber_min (a b : Ordinal) : ∗(min a b) = min (∗a) (∗b) := by sorry
...
theorem add_def (a b : Nimber) :
    a + b = sInf {x | (∃ a' < a, a' + b = x) ∨ ∃ b' < b, a + b' = x}ᶜ := by sorry
...
@[target]
theorem exists_of_lt_add (h : c < a + b) : (∃ a' < a, a' + b = c) ∨ ∃ b' < b, a + b' = c := by sorry
...
@[target]
theorem add_eq_zero {a b : Nimber} : a + b = 0 ↔ a = b := by sorry

@[target]
theorem add_ne_zero_iff : a + b ≠ 0 ↔ a ≠ b := by sorry

@[target, simp]
theorem add_self (a : Nimber) : a + a = 0 := by sorry
...
@[target, simp]
theorem add_cancel_right (a b : Nimber) : a + b + b = a := by sorry

@[target, simp]
theorem add_cancel_left (a b : Nimber) : a + (a + b) = b := by sorry

@[target]
theorem add_trichotomy {a b c : Nimber} (h : a + b + c ≠ 0) :
    b + c < a ∨ c + a < b ∨ a + b < c := by sorry

@[target]
theorem lt_add_cases {a b c : Nimber} (h : a < b + c) : a + c < b ∨ a + b < c := by sorry

/-- Nimber addition of naturals corresponds to the bitwise XOR operation. -/
@[target]
theorem add_nat (a b : ℕ) : ∗a + ∗b = ∗(a ^^^ b) := by sorry
