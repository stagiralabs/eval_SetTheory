...
theorem mul_def (a b : Nimber) :
    a * b = sInf {x | ∃ a' < a, ∃ b' < b, a' * b + a * b' + a' * b' = x}ᶜ := by sorry
...
@[target]
theorem exists_of_lt_mul (h : c < a * b) : ∃ a' < a, ∃ b' < b, a' * b + a * b' + a' * b' = c := by sorry
...
@[target]
theorem zero_mem_invSet (a : Nimber) : 0 ∈ invSet a := by sorry
...
theorem cons_mem_invSet {a' : Nimber} (ha₀ : a' ≠ 0) (ha : a' < a) (hb : b ∈ invSet a) :
    invAux a' * (1 + (a + a') * b) ∈ invSet a := by sorry
...
@[target]
theorem mem_invSet_of_lt_invAux (h : b < invAux a) : b ∈ invSet a := by sorry

@[target]
theorem invAux_not_mem_invSet (a : Nimber) : invAux a ∉ invSet a := by sorry

@[target]
theorem invAux_mem_invSet_of_lt (ha : a ≠ 0) (hb : a < b) : invAux a ∈ invSet b := by sorry
...
@[target]
theorem mul_invAux_cancel (h : a ≠ 0) : a * invAux a = 1 := by sorry
