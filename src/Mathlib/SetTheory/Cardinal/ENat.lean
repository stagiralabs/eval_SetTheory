...
@[target]
lemma ofENat_strictMono : StrictMono ofENat := by sorry
...
lemma ofENat_lt_aleph0 {m : ℕ∞} : (m : Cardinal) < ℵ₀ ↔ m < ⊤ := by sorry
...
@[target]
lemma ofENat_mono : Monotone ofENat := by sorry
...
lemma ofNat_le_ofENat {m : ℕ} [m.AtLeastTwo] {n : ℕ∞} :
    (ofNat(m) : Cardinal) ≤ n ↔ OfNat.ofNat m ≤ n := by sorry

@[target]
lemma ofENat_injective : Injective ofENat := by sorry

@[target, simp, norm_cast]
lemma ofENat_inj {m n : ℕ∞} : (m : Cardinal) = n ↔ m = n := by sorry
...
@[target, simp]
lemma range_ofENat : range ofENat = Iic ℵ₀ := by sorry
...
@[target]
lemma toENatAux_nat (n : ℕ) : toENatAux n = n := by sorry

@[target]
lemma toENatAux_zero : toENatAux 0 = 0 := by sorry

@[target]
lemma toENatAux_eq_top {a : Cardinal} (ha : ℵ₀ ≤ a) : toENatAux a = ⊤ := by sorry
...
@[target]
lemma toENatAux_gc : GaloisConnection (↑) toENatAux := by sorry

@[target]
theorem toENatAux_le_nat {x : Cardinal} {n : ℕ} : toENatAux x ≤ n ↔ x ≤ n := by sorry

@[target]
lemma toENatAux_eq_nat {x : Cardinal} {n : ℕ} : toENatAux x = n ↔ x = n := by sorry

@[target]
lemma toENatAux_eq_zero {x : Cardinal} : toENatAux x = 0 ↔ x = 0 := by sorry
...
@[target]
lemma enat_gc : GaloisConnection (↑) toENat := by sorry
...
@[target]
lemma toENat_strictMonoOn : StrictMonoOn toENat (Iic ℵ₀) := by sorry

@[target]
lemma toENat_injOn : InjOn toENat (Iic ℵ₀) := by sorry

@[target]
lemma ofENat_toENat_le (a : Cardinal) : ↑(toENat a) ≤ a := by sorry

@[target, simp]
lemma ofENat_toENat_eq_self {a : Cardinal} : toENat a = a ↔ a ≤ ℵ₀ := by sorry

@[target, simp] alias ⟨_, ofENat_toENat⟩ := ofENat_toENat_eq_self
...
@[target, simp] lemma toENat_eq_top {a : Cardinal} : toENat a = ⊤ ↔ ℵ₀ ≤ a := enat_gc.u_eq_top
...
@[target]
theorem toENat_congr {α : Type u} {β : Type v} (e : α ≃ β) : toENat #α = toENat #β := by sorry

@[target]
lemma toENat_le_iff_of_le_aleph0 {c c' : Cardinal} (h : c ≤ ℵ₀) :
    toENat c ≤ toENat c' ↔ c ≤ c' := by sorry
...
@[target, simp, norm_cast]
lemma ofENat_add (m n : ℕ∞) : ofENat (m + n) = m + n := by sorry
